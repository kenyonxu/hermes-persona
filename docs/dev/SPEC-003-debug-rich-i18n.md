...
**日期**: 2026-05-20  
**作者**: 知惠 (zhihui)  
**修订**: 适配 PLAN-004 目录扁平化重构 + CR-014 config 模块提取  
...
| 文件 | 改动类型 |
|------|---------|
| `injector.py` → `_debug_summary()` | 重构为 compact/detailed 双模式 |
| `injector.py` → `transform_llm_output()` | 无改动 |
| `injector.py` → 新增 `_detailed_summary()` | 新函数 |
| `locales/zh.json` | 新建 |
| `locales/en.json` | 新建 |
| `locales/__init__.py` | 新建（加载器） |
| `config.py` | 依赖（CR-014 提取的共享常量模块），`_CONFIG_ROOT` 等 |
| `examples/persona-config.json` | 更新范例 |
| `tests/test_debug_detailed.py` | 新建 |
| `tests/test_locales.py` | 新建 |

> **config 模块说明**：CR-014 将共享常量（`_CONFIG_ROOT` 等）提取到根目录 `config.py`。`injector.py` 和 `guard.py` 通过 `__init__.py` 注册的 `sys.modules` 短别名（`import config`）或绝对导入访问，无需 `hermes_persona.` 前缀。
