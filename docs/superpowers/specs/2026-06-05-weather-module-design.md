...
3. `_weather_context()` 检测到缓存中的 `location` 与新配置不匹配
4. 触发 _should_refresh → 清除旧坐标 → 重新 geocoding → 获取新天气 → 写入新缓存
5. 本轮注入即显示新地点的天气

整个流程依赖两个机制：(a) `_load_config()` 每轮重新读取配置文件；(b) 缓存文件中记录了上次查询的 `location`，与当前配置比对即可检测变更。
