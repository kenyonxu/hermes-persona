...
核心思想：所有「是否需要刷新」的判断统一收敛到 `_should_refresh()` 一个函数。流程如下：
...
_weather_context(config) → str | None
  ...
  │
  ├─ _should_refresh(cache, config, location)  ← 统一决策点
  │   │
  │   │  返回 True（需要刷新）:
  │   │    ├─ 缓存不存在
  │   │    ├─ 缓存已过期（超过 cache_ttl_minutes）
  │   │    └─ location 变更（与缓存中的 location 不一致）
  │   │
  │   └─ 返回 False（缓存有效）→ 从缓存格式化 → 返回格式化字符串
  │
  ├─ API 调用（_should_refresh 为 True 时进入）
  │   ├─ 无坐标缓存 → geocoding API（获取坐标 + 缓存坐标）
  │   ├─ weather API → 天气数据
  │   ├─ 写入缓存文件
  │   └─ 返回格式化字符串
  │
  └─ 任何 API/IO 失败→ 有旧缓存时回退旧缓存，无缓存时返回 None（fail-open）
```

`_should_refresh(cache, config, location)` 是纯函数，仅做条件判断，不产生副作用。三个触发条件涵盖所有需要重新获取天气的场景——包括运行时用户让 Agent 修改配置导致的 location 变更（见 4.7）。

### 4.3 缓存文件结构

缓存文件路径为插件自身的 `state/` 目录：

```
{plugin_dir}/state/weather_cache.json
```

即 `Path(__file__).resolve().parent / "state" / "weather_cache.json"`，与现有的 `state/reply_timing.json`、`state/daily_turn_count.json` 使用相同目录。
...
当用户让 Agent 修改 `persona-config.json` 中的 `location` 时（例如 "我要去上海出差，帮我把天气地点改一下"），变更在下一轮自动生效，无需重启：

1. Agent 编辑 `{profile}/plugins/hermes-persona/persona-config.json`，将 `weather.location` 改为新城市
2. 下一轮 `inject_context()` 调用时，`_load_config()` 重新读取配置文件，获取新的 `location`
3. `_weather_context()` 读取缓存后调用统一的 `_should_refresh()`
4. `_should_refresh()` 发现缓存中的 `location` 与新配置不匹配 → 返回 `True`
5. 触发 API 流程：清除旧坐标 → 重新 geocoding → 获取新天气 → 写入新缓存
6. 本轮注入即显示新地点的天气

整个流程依赖两个机制：(a) `_load_config()` 每轮重新读取配置文件；(b) 缓存中记录了上次的 `location`，`_should_refresh` 统一比对，location 变更和 TTL 过期走完全相同的代码路径。
...
def _should_refresh(cache: dict, config: dict, location: str) -> bool:
    ...
    返回 True 的条件（任一满足）：
    - 缓存不存在（cache 为 None 或空）
    - TTL 过期（fetched_at 距今超过 cache_ttl_minutes）
    - location 变更（缓存中的 location 与当前配置不一致，含运行时热更新场景）
